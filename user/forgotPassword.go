	...
	if err := initializer.DB.First(&userCheck, "email=?", email).Error; err != nil {
		...
		if result.Error != nil {
			otpStore = models.OtpMail{
				...
				CreatedAt: time.Now(),
				ExpireAt:  time.Now().Add(600 * time.Second),
			}
...
func ForgotOtpCheck(c *gin.Context) {
	...
	fmt.Println("Received OTP:", userOTP)

	var existingOTP models.OtpMail
	if err := initializer.DB.Where("otp = ? AND expire_at > ?", userOTP, time.Now()).Find(&existingOTP).Error; err != nil {
